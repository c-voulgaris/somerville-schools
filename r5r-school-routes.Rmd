...
```{r, message=FALSE}
my_core <- here("existing-osm-network") |>
  setup_r5()
```
...
## Editing Level of Traffic Stress

the R5 engine is guessing level of traffic stress based on common tags in 
OpenStreetMap. You can read more details [here](https://docs.conveyal.com/learn-more/traffic-stress){target="_blank"}.

You can use [JOSM](https://josm.openstreetmap.de/wiki/Download){target="_blank"}
to edit an *.osm file and set the level of traffic stress explicitly for use
with R5. Open the *.osm file in JOSM and add a key/value tag of lts=1 to set 
the level of traffic stress for a link to LTS 1.

For your convenience, I've included a copy of the josm executable in this 
GitHub repository. I've also included am executable that will convert an 
\*.osm file to a \*.pbf file for you (these are both for use with a PC).
